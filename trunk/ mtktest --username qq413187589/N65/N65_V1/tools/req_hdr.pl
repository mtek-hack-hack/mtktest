...
#
#!/usr/bin/perl
# find out required .(hpp|h|dat|c|hex|incl) for the header sources listed in .lis and in .dep

# show usage
sub usage
{
	print "USAGE: perl req_hdr.pl <filename of .lis> <filename of .dep>\n";
	exit(0);
}
...
#         print "\n\n$dep_count\t";
         while ($reading =~ /\b\S+\.(hpp|h|dat|c|hex|incl)/)      # find out next .(hpp|h|dat|c|hex) in this line
         {
            $hdr_name = "\L$&";
            $reading = ($`).($');
...
#                  print "[$hdr_name]\t";
   ...
   elsif (/\.(hpp|h|dat|c|hex|incl)/ && $needed_obj == 1)
   {
      $reading = $_;
...
print "\n\n$hdr_count .(hpp|h|dat|c|hex|incl) will be copied: \n";
