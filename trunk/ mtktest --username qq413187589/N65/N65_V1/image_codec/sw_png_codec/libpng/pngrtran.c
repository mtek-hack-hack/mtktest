...
// kumar: affected members are color_type, bit_depth, num_trans, pixel_depth, rowbytes
void /* PRIVATE */
            ...
            switch (row_info->bit_depth)
            {
               case 1:
               {
                  ...
                  gray = (png_uint_16)(gray*0xff);
               	// sp pointer to the last byte of the row(before transform)
                  sp = row + (png_size_t)((row_width - 1) >> 3);
               	// dp pointer to the rightmost destination byte position(after transform)
                  dp = row + (png_size_t)row_width - 1;
               	// shift out the residue
                  shift = 7 - (int)((row_width + 7) & 0x07);
                  for (i = 0; i < row_width; i++)
                  {
                     if ((*sp >> shift) & 0x01)
