...
const kal_uint8 ADC_ACCESSORYID = 5;//tmp solution, maybe SA will add this 
const kal_uint8 ADC_PCBTMP = 4;
#if defined(__CHARGER_USB_DETECT_WIHT_ONE_EINT__)
const kal_uint8 ADC_CHR_USB = 6;  // adc to distinguish between charger with usb
#endif
...
kal_uint8 custom_adc_get_channel(adc_channel_type type)
{
   switch(type)
   {
      case vbat_adc_channel:
         return ((kal_uint8)ADC_VBAT);
      case visense_adc_channel:
         return ((kal_uint8)ADC_VISENSE);
