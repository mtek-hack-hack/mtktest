...
#define GC0307_WRITE_ID						    0x42
#define GC0307_READ_ID								0x43
#define GC0307_SENSOR_ID							0x99	

extern kal_uint8  start_grab_x_offset, start_grab_y_offset;
kal_bool   gVGAmode, sensor_night_mode, MPEG4_encode_mode, g_bMJPEG_mode ;
...
void Delayms_GC0307(kal_uint32 ms)
{
         kal_uint32 delay;
...
void set_GC0307_dummy(kal_uint16 pixels, kal_uint16 lines)
{
...
/*************************************************************************
* FUNCTION
*	config_GC0307_window
*
* DESCRIPTION
*	This function config the hardware window of GC0307 for getting specified
*  data of that window.
*
* PARAMETERS
...
*
*************************************************************************/
void config_GC0307_window(kal_uint16 startx,kal_uint16 starty,kal_uint16 width, kal_uint16 height)
{
...
*
*************************************************************************/
void get_GC0307_id(kal_uint8 *sensor_write_id, kal_uint8 *sensor_read_id)
{
	*sensor_write_id=GC0307_WRITE_ID;
	*sensor_read_id=GC0307_READ_ID;
}	/* get_GC0307_id */
...
*
*************************************************************************/
void get_GC0307_size(kal_uint16 *sensor_width, kal_uint16 *sensor_height)
{
...
*
*************************************************************************/
void get_GC0307_period(kal_uint16 *pixel_number, kal_uint16 *line_number)
{
...
void GC0307_preview(image_sensor_exposure_window_struct *image_window, image_sensor_config_struct *sensor_config_data)
{
    ...
    // 1. GC0307's clock control register causes sensor output some abnormal frame even if
...
//	}
		...
		(sensor_config_data->isp_op_mode==ISP_MJPEG_ENCODE_MODE))  //// MPEG4 Encode Mode
	{
			...
			write_cmos_sensor(	0xd6  ,0x3c); 
			write_cmos_sensor(	0x28  ,0x02); 
			write_cmos_sensor(	0x29  ,0x58); 
			write_cmos_sensor(	0x2a  ,0x02); 
			write_cmos_sensor(	0x2b  ,0x58); 
			write_cmos_sensor(	0x2c  ,0x02); 
			write_cmos_sensor(	0x2d  ,0x58); 
			write_cmos_sensor(	0x2e  ,0x05); 
			write_cmos_sensor(	0x2f  ,0xa0); 
			
			write_cmos_sensor(	0x7e  ,0x45); 
			write_cmos_sensor(	0x7f  ,0x66); 
			write_cmos_sensor(	0x5c  ,0x48); 
			write_cmos_sensor(	0x5d  ,0x58); 			
    ...
    {
		    MPEG4_encode_mode=KAL_FALSE;
			write_cmos_sensor(	0x01  ,0x6a); 
			write_cmos_sensor(	0x02  ,0x25); 
			write_cmos_sensor(	0x10  ,0x00); 
			write_cmos_sensor(	0xd6  ,0x4b); 
			write_cmos_sensor(	0x28  ,0x02); 
			write_cmos_sensor(	0x29  ,0x0d); 
			write_cmos_sensor(	0x2a  ,0x02); 
			write_cmos_sensor(	0x2b  ,0x0d); 
			write_cmos_sensor(	0x2c  ,0x02); 
			write_cmos_sensor(	0x2d  ,0x0d); 
			write_cmos_sensor(	0x2e  ,0x05); 
			write_cmos_sensor(	0x2f  ,0xdc); 
	...
	switch (sensor_config_data->image_mirror)
	{
		...
		image_window->grab_start_y=8;//IMAGE_SENSOR_VGA_INSERTED_LINES+dummy_lines;	
...
void GC0307_capture(image_sensor_exposure_window_struct *image_window, image_sensor_config_struct *sensor_config_data)
{
	...
	{	/* Less than VGA Mode */
		...
		image_window->grab_start_x=4;////IMAGE_SENSOR_VGA_INSERTED_PIXELS;
		image_window->grab_start_y=8;//IMAGE_SENSOR_VGA_INSERTED_LINES+dummy_lines;	
		image_window->exposure_window_width=IMAGE_SENSOR_VGA_WIDTH-16;  //IMAGE_SENSOR_VGA_WIDTH
		image_window->exposure_window_height=IMAGE_SENSOR_VGA_HEIGHT-16;  //IMAGE_SENSOR_VGA_HEIGHT-4
...
*
* PARAMETERS
*	addr : the register index of GC0307
*  para : setting parameter of the specified register of GC0307
*
* RETURNS
*	None
*
...
*
*************************************************************************/
void write_GC0307_reg(kal_uint32 addr, kal_uint32 para)
{
...
*
*************************************************************************/
kal_uint32 read_GC0307_reg(kal_uint32 addr)
{
...
*
* DESCRIPTION
*	This function set e-shutter of GC0307 to change exposure time.
*
* PARAMETERS
...
*
*************************************************************************/
void set_GC0307_shutter(kal_uint16 shutter)
{
...
*
*************************************************************************/
kal_uint16 set_GC0307_gain(kal_uint16 gain)
{
...
*
*************************************************************************/
void GC0307_night_mode(kal_bool enable)
{
	kal_uint16 temp=0;
	temp = read_cmos_sensor(0x41);
    if(enable)
	{
		MAX_EXPOSURE_LINES=2*(PIXEL_CLK/MIN_FRAME_RATE)/VGA_PERIOD_PIXEL_NUMS;

		sensor_night_mode=KAL_TRUE;
//		write_cmos_sensor(	0x59  ,0x07);  // close auto PGA offset
		write_cmos_sensor(	0xdd  ,0x32);
		write_cmos_sensor(	0x41  ,temp & ~0x20);
		write_cmos_sensor(	0xb0  ,0x10);
		write_cmos_sensor(	0x21  ,0xf0);
...
//		write_cmos_sensor(	0x59  ,0x07);  // close auto PGA offset		
		write_cmos_sensor(	0xdd  ,0x22);  //0x12
		write_cmos_sensor(	0x41  ,temp | 0x20); 
...
/*************************************************************************
* FUNCTION
*	set_GC0307_flashlight
*
* DESCRIPTION
*	turn on/off GC0307 flashlight .
*
* PARAMETERS
*	none
*
...
*
*************************************************************************/
void set_GC0307_flashlight(kal_bool enable)
{
	// Todo
}
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_zoom(kal_uint32 para)
{
	return KAL_FALSE;
}
...
/*************************************************************************
* FUNCTION
*	set_GC0307_param_contrast
*
* DESCRIPTION
*	GC0307 contrast setting.
*
* PARAMETERS
*	none
*
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_contrast(kal_uint32 para)
{
	// Not Support
	return KAL_FALSE;	
}
...
/*************************************************************************
* FUNCTION
*	set_GC0307_param_brightness
*
* DESCRIPTION
*	GC0307 brightness setting.
*
* PARAMETERS
*	none
*
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_brightness(kal_uint32 para)
{
	// Not Support
	return KAL_FALSE;	
}
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_hue(kal_uint32 para)
{
	// Not Support
	return KAL_FALSE;	
}
...
/*************************************************************************
* FUNCTION
*	set_GC0307_param_gamma
*
* DESCRIPTION
*	GC0307 gamma setting.
*
* PARAMETERS
*	none
*
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_gamma(kal_uint32 para)
{
	return KAL_FALSE;	
}
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_wb(kal_uint32 para)
{
//	kal_uint16 rgain=0x80, ggain=0x80, bgain=0x80;
	...
	{
		case CAM_WB_AUTO:
			write_cmos_sensor(0xc7,0x4c); //for AWB can adjust back
			write_cmos_sensor(0xc8,0x40);
			write_cmos_sensor(0xc9,0x4a);			
		...
		case CAM_WB_CLOUD:
			write_cmos_sensor(0x41,temp_reg&~0x04);   // Enable AWB 
			write_cmos_sensor(0xc7,0x5a); //WB_manual_gain
			write_cmos_sensor(0xc8,0x42);
			write_cmos_sensor(0xc9,0x40);
		...
		case CAM_WB_DAYLIGHT:   // tai yang guang
			...
			write_cmos_sensor(0xc8,0x45);
			write_cmos_sensor(0xc9,0x40);			
			break;		

		case CAM_WB_INCANDESCENCE:   // bai re guang
			write_cmos_sensor(0x41,temp_reg&~0x04);   // Enable AWB 
			write_cmos_sensor(0xc7,0x48);
			write_cmos_sensor(0xc8,0x40);
			write_cmos_sensor(0xc9,0x5c);

			break;		

		case CAM_WB_FLUORESCENT:   //ri guang deng
			write_cmos_sensor(0x41,temp_reg&~0x04);   // Enable AWB 
			write_cmos_sensor(0xc7,0x40);
			write_cmos_sensor(0xc8,0x42);
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_exposure(kal_uint32 para)
{
	switch (para)
	{
		case CAM_EV_NEG_4_3:    
...
kal_uint32 set_GC0307_param_effect(kal_uint32 para)
{
	...
	temp_reg=read_cmos_sensor(0x47);

	switch (para)
	{	
		case CAM_EFFECT_ENC_NORMAL:
			if(sensor_night_mode)
				write_cmos_sensor(0x41,0x0f);
			else
				write_cmos_sensor(0x41,0x2f);			
			write_cmos_sensor(0x40,0x7e);
			write_cmos_sensor(0x42,0x10);
			//write_cmos_sensor(0x47,0x2c);
			write_cmos_sensor(0x47,temp_reg & 0xef);
			write_cmos_sensor(0x48,0xc3);
			write_cmos_sensor(0x8a,0x50);//60
		...
		case CAM_EFFECT_ENC_GRAYSCALE:
			...
			write_cmos_sensor(0x40,0x7e);
			write_cmos_sensor(0x42,0x10);
			write_cmos_sensor(0x47,temp_reg | 0x10);
			write_cmos_sensor(0x48,0xc3);
		...
		case CAM_EFFECT_ENC_COLORINV:	
			if(sensor_night_mode)
				write_cmos_sensor(0x41,0x4f);
			else
				write_cmos_sensor(0x41,0x6f);
			//write_cmos_sensor(0x41,0x6f);//[6]			
			write_cmos_sensor(0x40,0x7e);
			write_cmos_sensor(0x42,0x10);
			//write_cmos_sensor(0x47,0x2c);
			write_cmos_sensor(0x47,temp_reg & 0xef);
		...
		case CAM_EFFECT_ENC_SEPIABLUE:
			...
			write_cmos_sensor(0xa1,0x40);
			write_cmos_sensor(0x7a,0x00);
			write_cmos_sensor(0x78,0x70);
			write_cmos_sensor(0x79,0x00);
			write_cmos_sensor(0x7b,0x3f);
			write_cmos_sensor(0x7c,0xf5);					
			gc_effect_on = KAL_TRUE;
			break;								
		...
		case CAM_EFFECT_ENC_CONTRAST:
			...
			write_cmos_sensor(0x8c,0x00);
			write_cmos_sensor(0x50,0x08);
			write_cmos_sensor(0xdb,0x4a);
			write_cmos_sensor(0xb0,0xff);
			write_cmos_sensor(0x77,0x93);
			write_cmos_sensor(0xa1,0xff);
			write_cmos_sensor(0x7a,0x50);
			write_cmos_sensor(0x78,0x00);
			write_cmos_sensor(0x79,0x00);
		...
		case CAM_EFFECT_ENC_BLACKBOARD:
			...
			write_cmos_sensor(0x8c,0x00);
			write_cmos_sensor(0x50,0x08);
			write_cmos_sensor(0xdb,0x46);
			write_cmos_sensor(0xb0,0xff);
			write_cmos_sensor(0x77,0xa8);
			write_cmos_sensor(0xa1,0xff);
...
/*************************************************************************
* FUNCTION
*	set_GC0307_param_banding
*
* DESCRIPTION
*	GC0307 banding setting.
*
* PARAMETERS
*	none
*
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_banding(kal_uint32 para)
{
	kal_uint8 banding;
#if 0	
	switch (para)
...
/*************************************************************************
* FUNCTION
*	set_GC0307_param_saturation
*
* DESCRIPTION
*	GC0307 SATURATION setting.
*
* PARAMETERS
*	none
*
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_saturation(kal_uint32 para)
{
    // Not Support
	return KAL_FALSE;	
}
...
/*************************************************************************
* FUNCTION
*	set_GC0307_param_nightmode
*
* DESCRIPTION
*	GC0307 night mode setting.
*
* PARAMETERS
*	none
*
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_nightmode(kal_uint32 para)
{
	GC0307_night_mode((kal_bool)para);
	return KAL_TRUE;	
}
...
*
*************************************************************************/
kal_uint32 set_GC0307_param_ev(kal_uint32 para)
{
	return set_GC0307_param_exposure(para);
	return KAL_TRUE;
}

/*************************************************************************
* FUNCTION
*	GC0307_yuv_sensor_setting
*
* DESCRIPTION
...
*
*************************************************************************/
kal_uint32 GC0307_yuv_sensor_setting(kal_uint32 cmd, kal_uint32 para)
{
	kal_uint32 ret = KAL_TRUE;
	switch (cmd)
	{
		case CAM_PARAM_ZOOM_FACTOR:
			ret = set_GC0307_param_zoom(para);		
			break;			
		case CAM_PARAM_CONTRAST:
			ret = set_GC0307_param_contrast(para);		
			break;					
		case CAM_PARAM_BRIGHTNESS:
			ret = set_GC0307_param_brightness(para);		
			break;							
		case CAM_PARAM_HUE:
			ret = set_GC0307_param_hue(para);		
			break;							
		case CAM_PARAM_GAMMA:
			ret = set_GC0307_param_gamma(para);		
			break;							
		case CAM_PARAM_WB:
			ret = set_GC0307_param_wb(para);		
			break;
		case CAM_PARAM_EXPOSURE:
			ret = set_GC0307_param_exposure(para);		
			break;							
		case CAM_PARAM_EFFECT:
			ret = set_GC0307_param_effect(para);
			break;
		case CAM_PARAM_BANDING:
			ret = set_GC0307_param_banding(para);		
			break;		
		case CAM_PARAM_SATURATION:
			ret = set_GC0307_param_saturation(para);				
			break;						
		case CAM_PARAM_NIGHT_MODE:
			ret = set_GC0307_param_nightmode(para);				
			break;						
		case CAM_PARAM_EV_VALUE:
			ret = set_GC0307_param_ev(para);				
			break;				
		default:
			ret = KAL_FALSE;
	}
...
*
*************************************************************************/
image_sensor_func_struct image_sensor_func_GC0307=
{
	init_GC0307,
	get_GC0307_id,
	get_GC0307_size,
...
void camera_para_to_sensor_GC0307(void)
{
	kal_uint32	i;
	
	for(i=0; 0xFFFFFFFF!=camera_para.SENSOR.reg[i].addr; i++)
	{
		write_GC0307_reg(camera_para.SENSOR.reg[i].addr, camera_para.SENSOR.reg[i].para);
	}
	...
	{
		write_GC0307_reg(camera_para.SENSOR.cct[i].addr, camera_para.SENSOR.cct[i].para);
	}
}

// update camera_para from sensor register
void sensor_to_camera_para_GC0307(void)
{
	kal_uint32	i;
	
	for(i=0; 0xFFFFFFFF!=camera_para.SENSOR.reg[i].addr; i++)
	{
		camera_para.SENSOR.reg[i].para = read_GC0307_reg(camera_para.SENSOR.reg[i].addr);
	}
	for(i=FACTORY_START_ADDR; 0xFFFFFFFF!=camera_para.SENSOR.reg[i].addr; i++)
	{
		camera_para.SENSOR.reg[i].para = read_GC0307_reg(camera_para.SENSOR.reg[i].addr);
...
void  get_sensor_group_count_GC0307(kal_int32* sensor_count_ptr)
{
...
void get_sensor_group_info_GC0307(kal_uint16 group_idx, kal_int8* group_name_ptr, kal_int32* item_count_ptr)
{
	/*switch (group_idx)
	{
		case AWB_GAIN:
...
void get_sensor_item_info_GC0307(kal_uint16 group_idx,kal_uint16 item_idx, ENG_sensor_info* info_ptr)
{
	kal_uint8 temp_reg;
		...
		case AWB_GAIN:
			switch (item_idx)
			{
				case 0:
				  ...
				  info_ptr->item_value=read_GC0307_reg(camera_para.SENSOR.reg[AWB_GAIN_R_INDEX].addr);
				...
				case 1:
				  ...
				  info_ptr->item_value=read_GC0307_reg(camera_para.SENSOR.reg[AWB_GAIN_B_INDEX].addr);
				  info_ptr->is_true_false=KAL_FALSE;
				  info_ptr->is_read_only=KAL_FALSE;
				  info_ptr->is_need_restart=KAL_FALSE;
				  info_ptr->min=0;
		...
		case PRE_GAIN:
			switch (item_idx)
			{
				case 0:
				  ...
				  temp_reg = read_GC0307_reg(camera_para.SENSOR.cct[PRE_GAIN_R_INDEX].addr);
				  temp_reg &= 0x30;
				  temp_reg >>= 4; 
				...
				case 1:
				  ...
				  temp_reg = read_GC0307_reg(camera_para.SENSOR.cct[PRE_GAIN_B_INDEX].addr);
				  temp_reg &= 0xC0;
				  temp_reg >>= 6; 
		...
		case SENSOR_DBLC:
			switch (item_idx)
			{
				case 0:
				  ...
				  temp_reg = read_GC0307_reg(camera_para.SENSOR.reg[SENSOR_DBLC_INDEX].addr);
				  if(temp_reg&0x10)
				  {
		...
		case GAMMA_ENABLE:
			switch (item_idx)
			{
				case 0:
				  ...
				  temp_reg = read_GC0307_reg(camera_para.SENSOR.reg[GAMMA_ENABLE_INDEX].addr);
...
kal_bool set_sensor_item_info_GC0307(kal_uint16 group_idx, kal_uint16 item_idx, kal_int32 item_value)
{
   kal_uint8 temp_reg;
   ...
   /*switch (group_idx)
	{
		case AWB_GAIN:
			switch (item_idx)
			{
				case 0:
				  ...
				  write_GC0307_reg(camera_para.SENSOR.reg[AWB_GAIN_R_INDEX].addr,item_value);
				break;
				case 1:
				  camera_para.SENSOR.reg[AWB_GAIN_B_INDEX].para = item_value;
				  write_GC0307_reg(camera_para.SENSOR.reg[AWB_GAIN_B_INDEX].addr,item_value);
				break; 
				default:
				   ASSERT(0);
			}			
		...
		case PRE_GAIN:
			switch (item_idx)
			{
				case 0:
				  ...
				  write_GC0307_reg(camera_para.SENSOR.cct[PRE_GAIN_R_INDEX].addr,temp_reg);
				break;
				case 1:
				  temp_reg = read_GC0307_reg(camera_para.SENSOR.cct[PRE_GAIN_B_INDEX].addr);
				  temp_reg &= ~0xC0;
				  ...
				  write_GC0307_reg(camera_para.SENSOR.cct[PRE_GAIN_B_INDEX].addr,temp_reg);
				break;
				case 2:
				  ...
				  write_GC0307_reg(camera_para.SENSOR.cct[GLOBAL_GAIN_INDEX].addr,temp_reg);
				  break;
				default:
				   ASSERT(0);	
			}
			...
			switch (item_idx)
			{
				case 0:
				  if(item_value==1)
				  {
				      ...
				      write_GC0307_reg(camera_para.SENSOR.reg[SENSOR_DBLC_INDEX].addr,temp_reg);
				  }
				  else
				  {
				      temp_reg = read_GC0307_reg(camera_para.SENSOR.reg[SENSOR_DBLC_INDEX].addr);
		...
		case GAMMA_ENABLE:
			switch (item_idx)
			{
				case 0:
				  ...
				  write_GC0307_reg(camera_para.SENSOR.reg[GAMMA_ENABLE_INDEX].addr,temp_reg);
				break;
				default:
				   ASSERT(0);
			}
		...
		case REGISTER_EDITOR:
			switch (item_idx)
			{
				...
				break;
				case 1:
				  write_GC0307_reg(FAC_SENSOR_REG,item_value);
				break;
				default:
				   ASSERT(0);		
			}
...
kal_bool is_sensor_ready_GC0307()
{
	REG_ISP_CMOS_SENSOR_MODE_CONFIG |= REG_CMOS_SENSOR_RESET_BIT;
	kal_sleep_task(2);	
	REG_ISP_CMOS_SENSOR_MODE_CONFIG &= ~REG_CMOS_SENSOR_RESET_BIT;
	...
	kal_sleep_task(2);	
	write_cmos_sensor(	0x18  ,0xf0);  
	kal_sleep_task(10);
	sensor_id=(read_cmos_sensor(0x00));

#ifdef NOKE_DEBUG
	noke_dbg_printf("\r CAMERA : is_sensor_ready_GC0307 : sensor_id = 0x%04x \n", sensor_id);
#endif
	
	if( GC0307_SENSOR_ID == sensor_id )
		return KAL_TRUE;
	else
		return KAL_FALSE;
}
...
*/
void main_camera_gc0307_init_func_default(void)
{
	...
	write_cmos_sensor(	0x02  ,0x0c); //VB
	...
	write_cmos_sensor(	0x8a  ,0x50);//60  										   
	write_cmos_sensor(	0x8b  ,0x50);//60  										   
	write_cmos_sensor(	0x8c  ,0x07);  												 				  
	...
	write_cmos_sensor(	0xc1  ,0x60); 
	write_cmos_sensor(	0xc2  ,0xf1);  //f1
	write_cmos_sensor(	0xc3  ,0x40);
	write_cmos_sensor(	0xc4  ,0x1c); //18//20
	write_cmos_sensor(	0xc5  ,0x56);  //33
	...
	//=========  aecT  
	write_cmos_sensor(	0x20  ,0x06);//0x02 
	write_cmos_sensor(	0x21  ,0xc0); 
	...
	write_cmos_sensor(	0xd2  ,0x61);//0xf2 	  
	write_cmos_sensor(	0xd4  ,0x96); 
	write_cmos_sensor(	0xd5  ,0x01); // william 0318
	write_cmos_sensor(	0xd6  ,0x96); //antiflicker_step 					   
	write_cmos_sensor(	0xd7  ,0x03); //AEC_exp_time_min ,william 20090312			   
...
/////  请注意，调整GC0307的镜像和翻转，需要同时修改三个寄存器，如下:
	write_cmos_sensor(	0x0F  ,0xb2); //CISCTL mode1	
	write_cmos_sensor(	0x45  ,0x27);
	write_cmos_sensor(	0x47  ,0x2c); 	
...
// IMAGE_HV_MIRROR:	   // 180
	...
	write_cmos_sensor(	0x44  ,0xE0);
	//write_cmos_sensor(	0x59  ,0x07);
}
/*
...
void sub_camera_gc0307_init_func_default(void)
{
	// Initail Sequence Write In.
