 ...
 * Description:
 * ------------
 *   G726 codec driver, including G726 16Kbps, 24Kbps, 32Kbps and 40Kbps, 
 *     also support VAD/DTX and CNG/PLC
 *
 * Author:
 * -------
 * -------
...
#if defined(G726_UNIT_TEST) && !defined(__MTK_TARGET__)
#include <stdio.h>
#include <string.h>
#include <assert.h>
#include <time.h>
...
//extern const unsigned char DefaultRCCoeff[10];
...
******************************************************************************
*/
static void _g726_cod_reset_( G726_Cod *cod, int rate, int reset_all )
{
      cod->rate = rate;
      switch( rate )
      {
         case 2:
            cod->compress = g726_16_encoder;
            break;
         case 3:
            cod->compress = g726_24_encoder;
            break;
         case 4:
            cod->compress = g726_32_encoder;
            break;
         case 5:
            cod->compress = g726_40_encoder;
            break;
         default:
            ASSERT(0);
            break;
      }
      g726_init_state( &cod->state );
}

static void _g726_dec_reset_( G726_Dec *dec, int rate, int reset_all )
{
      dec->rate = rate;
      switch( rate )
      {
         case 2:
            dec->expand = g726_16_decoder;
            break;
         case 3:
            dec->expand = g726_24_decoder;
            break;
         case 4:
            dec->expand = g726_32_decoder;
            break;
         case 5:
            dec->expand = g726_40_decoder;
            break;
         default:
            ASSERT(0);
            break;
      }
      g726_init_state( &dec->state );
}


unsigned int G726_GetInternalBufferSize(int is_encoder)
{
   unsigned int buffer_size;

   if( is_encoder==1 )
      buffer_size = sizeof(G726_Cod) + G729AB_GetInternalBufferSize(1);
   else
      buffer_size = sizeof(G726_Dec);

   return buffer_size;
}

unsigned int G726_GetSharedBufferSize(int is_encoder)
{
   if( is_encoder==1 )
...
void *G726_Open(int rate, int is_encoder, 
   ...
   ASSERT( internal_buffer_size>=G726_GetInternalBufferSize( is_encoder ) );

   if( is_encoder==1 )
   {
      G726_Cod *cod;

      cod = (G726_Cod *)internal_buffer;
      used_buffer_size = aligned_sizeof( G726_Cod );
      internal_buffer += aligned_sizeof( G726_Cod );

      memset( cod, 0, sizeof( G726_Cod ) );
      _g726_cod_reset_( cod, rate, 1 );
   ...
   else
   {
      ...
      dec = (G726_Dec *)internal_buffer;
      used_buffer_size = aligned_sizeof( G726_Dec );
      internal_buffer += aligned_sizeof( G726_Dec );

      memset( dec, 0, sizeof( G726_Dec ) );
      _g726_dec_reset_( dec, rate, 1 );
...
void G726_Close(void *data, int is_encoder)
{

}

int G726_Encode(void *data, short *speech_in, CODE *bits_out, int VAD_enable, int CNG_enable, int rate)
{
   G726_Cod *cod = (G726_Cod *)data;
   int num_octets;
   int vad_result = 1;
...
/* under construction !*/
   ...
   { // voice frame
      int i;
      CODE *ptr = bits_out;
      unsigned int code, offset = 0, nb = rate;
      short *pSample;
      ...
      if( cod->rate!=rate )
         _g726_cod_reset_( cod, rate, 0 );

#if defined(__G7XX_DEBUG__)
      num_octets = FRAME_LEN*sizeof(CODE);
#else
      num_octets = (FRAME_LEN*rate + 7)/8;
#endif
      memset( ptr, 0, num_octets );

      for( i=0; i<FRAME_LEN; i++ )
      {
         code = (unsigned int)cod->compress( (int)pSample[i], &cod->state );
#if defined(__G7XX_DEBUG__)
         *ptr++ = code;
#else
#if G726_ALIGN_LSB
         ...
         *ptr |= (unsigned char)code;
         if( (int)(nb+offset-8)>0 )
            *(++ptr) |=  (unsigned char)(code>>8);
         offset = (offset+nb)&7;
         if( offset==0 )
            ++ptr;
#else
         ...
         {
            *ptr <<= (8 - offset);
            *ptr |= code >> (nb+offset-8);
            code &= (1 << (nb+offset-8)) - 1;
            *(++ptr) = code;
         }
         offset = (offset + nb) & 7;
         if( offset == 0 )
...
int G726_Decode(void *data, CODE *bits_in, short *speech_out, int num_octets, short frame_erasure, int isCNPayload, int disable_enhancement)
{
   G726_Dec *dec = (G726_Dec *)data;
   int frame_type = 4;
   int i;
   int packetloss = 0;
...
#else
#ifndef AUTO_DETECT_CN
      if((num_octets >= (FRAME_LEN*2/8)) && (isCNPayload == 0) )
#else
      if((num_octets >= (FRAME_LEN*2/8)))
#endif
      { // voice frame
         int rate = num_octets*8/FRAME_LEN;
#endif
         CODE *ptr = bits_in;
         unsigned int code, offset = 0, nb = rate;
         ...
         for( i=0; i<FRAME_LEN; i++ )
         {
...
#if G726_ALIGN_LSB
            int shamt;
            code = ((unsigned int)(*ptr))>>(offset);
            if( (shamt=(8-offset))<nb )
               code |= ((unsigned int)(*(++ptr)))<<shamt;
            code &= (1<<nb)-1;
            offset = (offset+nb)&7;
            if( offset==0 )
               ++(ptr);
#else
            int shamt = (int)(nb+offset-8);
            if( shamt<=0 )
            {
               code = ((unsigned int)(*ptr))>>(-shamt);
               code &= (1 << nb) - 1;
            ...
            speech_out[i] = (short)dec->expand( (int)code, &dec->state );
         }
...
void main2(int argc, char **argv)
{
...
#else
   int N_code = FRAME_LEN*rate/8;
   CODE code_buffer[FRAME_LEN*5/8];
#endif
   int num_octets;
   void *data;
   ...
   if(is_encode != 0)
   {
      ...
      buffer_size = G726_GetInternalBufferSize(1);
      ASSERT(G726_GetSharedBufferSize(1) <= sizeof(shared_buffer_pool));
      data = G726_Open( rate, 1, buffer, buffer_size, shared_buffer_pool );
   }
   else
   {
      ...
      buffer = internal_buffer_pool;
      buffer_size = G726_GetInternalBufferSize(0);
      ASSERT(G726_GetSharedBufferSize(0) <= sizeof(shared_buffer_pool));
      data = G726_Open( rate, 0, buffer, buffer_size, shared_buffer_pool );
      postproc_init();
   }
...
#endif
            num_octets = G726_Encode( data, linear_buffer, code_buffer, vad_enable, 1, rate );
#if defined(__PROFILING__)
         ...
         if( nbytes==1 )
         {
            ...
            if( nbytes==num_octets )
            {
               ...
               if(sample_processed == stop_sample)
                  printf("stop_sample\n");
               Ftyp = G726_Decode( data, code_buffer, linear_buffer, num_octets, 0, (num_octets >= 1 && num_octets<20)? 1: 0 );
