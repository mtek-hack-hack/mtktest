...
png_handle_IHDR(png_structp png_ptr, png_infop info_ptr, png_uint_32 length)
{
   ...
   png_ptr->channels);
   // kumar: scan line always begin on byte boundary
...
png_combine_row(png_structp png_ptr, png_bytep row, int mask)
{
   ...
   if (mask == 0xff)
   {
   	// kumar ( noninterlace go through here)
   	extern kal_bool is_PNG_cliped;;

		if(is_PNG_cliped)
		{
			img_resizer_put_row(png_ptr->row_buf + 1, png_ptr->width, png_ptr->row_number,1);				
		}
		else
		{
         ...
         default:
         {
            ...
            for (i = 0; i < row_width; i++)
            {
               ...
               if (m & mask)
               {
               	if(is_PNG_cliped)
               	{
							img_resizer_put_pixel(i,png_ptr->row_number,sp);
						}
               	else
               	{
...
png_read_start_row(png_structp png_ptr)
{
   ...
   /* start of interlace block in the x direction (col)*/
   const int png_pass_start[7] = {0, 4, 0, 2, 0, 1, 0};

   /* offset to next interlace block (col) */
   const int png_pass_inc[7] = {8, 8, 4, 4, 2, 2, 1};

   /* start of interlace block in the y direction (row)*/
   const int png_pass_ystart[7] = {0, 0, 4, 0, 2, 0, 1};

   /* offset to next interlace block in the y direction (row) */
   const int png_pass_yinc[7] = {8, 8, 8, 4, 4, 2, 2};
#endif
   ...
   if (png_ptr->interlaced)
   {
      ...
      png_ptr->iwidth = (png_ptr->width +
         ...
         row_bytes = ((png_ptr->iwidth *
            (png_uint_32)png_ptr->pixel_depth + 7) >> 3) +1; // kumar: why adding 1(filter type byte)?
