...
# Constants
#****************************************************************************
my $EMIGEN_VERNO  = " V0.12";
                    # V0.12 , by JI at 2007/04/04 , flash_opt.h algo revise , add __RENESAS_64M__ conditional
                    # V0.11 , by JI at 2007/03/30 , custom_EMI.c template revise for MT6205B
                    # V0.09 , by JI at 2007/03/25 , flash_opt.h template revise , add BUFFER_PROGRAM_LENGTH define
                    # V0.08 , by JI at 2007/03/01 , flash_opt.h template revise
                    # V0.07 , by JI at 2007/02/13 , NOR_FLASH_SINGLE_BANK Support minor revise
                    # V0.06 , by JI at 2007/02/06 , NOR_FLASH_SINGLE_BANK Support for MT6225
                    # V0.05 , by JI at 2007/01/29 , NOR_FLASH_SINGLE_BANK Support for MT6227
                    # V0.04 , by JI at 2007/01/23 , bug fix for emi-setting lines
                    # V0.03 , by JI at 2007/01/19 , structural revise
                    #                  New API custom_get_NORFLASH_ROMSpace()
                    #                  NOR_FLASH_TYPE_SPANSION_NS_N support
                    # V0.02 , by JI at 2007/01/11 , 6225 columns typo fix
                    # V0.01 , rewritten version
...
my $COLUMN_ASYNC_DRIVING      	= 21;
my $COLUMN_52MHZ_SYNC_EMI       = 27;
my $COLUMN_52MHZ_SYNC_DRIVING   = 30;
my $COLUMN_104MHZ_SYNC_EMI    	= 33;
my $COLUMN_26MHZ_SYNC_EMI	= 36; # modified for clock switching
my $COLUMN_13MHZ_SYNC_EMI	= 39; # modified for clock switching
my $COLUMN_104MHZ_SYNC_DRIVING	= 42; # modified for clock switching
...
sub flash_opt_h_file_body
{
    ...
    if ( ($MEMORY_DEVICE_TYPE eq 'NOR_RAM_MCP') )
    {
        if ($part_info->{CS}->{0}->{PBP}->{Y_OR_N} =~ /Y/i)
        {
            ...
            $current_nor_opt .= "#define BUFFER_PROGRAM_LENGTH ($pbp_size)\n";
        }
        ...
        # additional option for RENESAS !
        if (defined $nor_series and $nor_series =~ /RENESAS/ and $part_info->{CS}->{0}->{SIZE} == 64)
        {
            $current_nor_opt .= "#define __RENESAS_64M__\n";
        }
...
{
    my $template = <<"__TEMPLATE";
#if defined(MT6225) && !defined(ADMUX_MCP)
...
sub HeaderBody_for_nor_ram_mcp
{
    ...
    my $sub_content_block3 = &emi_setting();
