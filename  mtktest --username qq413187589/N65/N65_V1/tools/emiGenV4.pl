...
# Constants
#****************************************************************************
my $EMIGEN_VERNO  = " V4.17";
                    # V4.17 , by Claudia at 2007/03/12 , Modify check mechanism when both EXPECTED_CODE_SIZE and EXPECTED_FAT_SIZE are set
                    # V4.16 , by JI at 2007/12/28 , custom_switchclock.c LPSDRAM enable CKE
                    # V4.15 , by JI at 2007/12/27 , custom_switchclock.c 26M block template updated
                    # V4.14 , by JI at 2007/12/24 , Additional NOR_RAM_MCP Configuration Instructions For EMI/SYS AutoGen
                    #                               Bank info correction
                    # V4.13 , by JI at 2007/12/03 , custom_switchclock.c FuncBody_custom_dynamicclockswitch_INTELSIBLEY
                    #                               output 1st EMI_ASYNC setting
                    # V4.12 , by JI at 2007/11/19 , New compile option __UBL__ 
                    # V4.11 , by JI at 2007/11/07 , Additional NOR_RAM_MCP Configuration Instructions For EMI/SYS AutoGen
                    #                               Project Expected CODE/RAM/FAT Size
                    # V4.09 , by JI at 2007/11/04 , clear custom_EMI.h block size information
                    # V4.07 , by JI at 2007/09/10 , adptive to new excel header
                    # V4.06 , by JI at 2007/08/14 , Device Excel data base extended
...
my $COLUMN_BANK_INFO            = 15;
my $COLUMN_BLOCK_INFO           = 16;
my $COLUMN_ASYNC_EMI            = 17;
my $COLUMN_ASYNC_DRIVING        = 24;
my $COLUMN_52MHZ_SYNC_EMI       = 31;
my $COLUMN_52MHZ_SYNC_DRIVING   = 36;
my $COLUMN_104MHZ_SYNC_EMI      = 41;
my $COLUMN_26MHZ_SYNC_EMI       = 44; # modified for clock switching
my $COLUMN_13MHZ_SYNC_EMI       = 47; # modified for clock switching
my $COLUMN_104MHZ_SYNC_DRIVING  = 52; # modified for clock switching
...
my $PROJECT_EXPECTED_CODESIZE;
my $PROJECT_EXPECTED_RAMSIZE;
my $PROJECT_EXPECTED_FATSIZE;
...
while (<CUSTOM_MEMORY_DEVICE_HDR>)
{
   ...
   # "#define CS0_PART_NUMBER       K5L2731CAA0"
   # "!NOR_RAM_MCP PROJECT EXPECTED CODESIZE   0x000E0000"
   # "!NOR_RAM_MCP PROJECT EXPECTED RAMSIZE    0x00040000"
   # "!NOR_RAM_MCP PROJECT EXPECTED FATSIZE    0x00020000"
   ...
   $PROJECT_EXPECTED_CODESIZE       = $1 if (/^!$MEMORY_DEVICE_TYPE\s+PROJECT EXPECTED CODESIZE\s+(\w+)/);
   $PROJECT_EXPECTED_RAMSIZE        = $1 if (/^!$MEMORY_DEVICE_TYPE\s+PROJECT EXPECTED RAMSIZE\s+(\w+)/);
   $PROJECT_EXPECTED_FATSIZE        = $1 if (/^!$MEMORY_DEVICE_TYPE\s+PROJECT EXPECTED FATSIZE\s+(\w+)/);
}
...
if ($DebugPrint == 1)
{
   ...
   print "Manual Configure ( $PROJECT_EXPECTED_CODESIZE , $PROJECT_EXPECTED_RAMSIZE , $PROJECT_EXPECTED_FATSIZE )\n";
}
  ...
  && $BBtbl_SYNC_ACCESS{$PLATFORM} )
{
    ...
    {
        die "Device not support 104MHz Burst Access!\n" if ($part_info->{CS}->{0}->{C104MHZ_SYNC_EMI}->{DATA} eq 'x');
        die "Device not support 104MHz Burst Access!\n" if ($part_info->{CS}->{1}->{C104MHZ_SYNC_EMI}->{DATA} eq 'x');
    ...
    {
        die "Device not support 52MHz Burst Access!\n" if ($part_info->{CS}->{0}->{C52MHZ_SYNC_EMI}->{DATA} eq 'x');
        die "Device not support 52MHz Burst Access!\n" if ($part_info->{CS}->{1}->{C52MHZ_SYNC_EMI}->{DATA} eq 'x');
...
#****************************************************************************
# subroutine:  custom_setadvemi_for_52MHz_sync_access
# return:      template of "FUNCTION custom_setAdvEMI()" for 28/29 SYNC_ACCESS
#              part 1
#****************************************************************************
sub custom_setadvemi_for_52MHz_sync_access
{
    my @data_array_on_flash = split /\s/, $part_info->{CS}->{0}->{C52MHZ_SYNC_EMI}->{DATA};
    my @data_array_on_ram   = split /\s/, $part_info->{CS}->{1}->{C52MHZ_SYNC_EMI}->{DATA};
    my $sub_content1;
    my $sub_content2;
    ###

    for (0..$#data_array_on_flash)
    {
        my $idx = $_ + 1;
...
$sub_content2
      ...
      *EMI_GEN  = EMI_DRV0_BURST_MODE;
#else
      *EMI_GENA = EMI_DRV0_BURST_MODE;
#endif

#if ( defined(MT6228) || defined(MT6229) || defined(MT6230) || defined(MT6268T) )
...
sub bank_info
{
    my ($bank_aref, $offset, $fatsz) = @_;
    my $template;

    for (0..$#$bank_aref)
    {
        if ($bank_aref->[$_] =~ /\{(0x\w+),\s*(\d+)\}/) # match {0x20000, 7}
        {
            my $segment = ( hex($1) * $2 );
            if ($offset <= 0)
            # Bank already in 
            {
                ...
                # Bank full in-used
                {
                    $template .= sprintf("   \{ 0x%X, %d \},\n", hex($1), $2);
                }
                elsif ($fatsz > 0)
                # Bank partial used
                {
                    if ( int($fatsz / hex($1)) > 0 )
                    {
                    $template .= sprintf("   \{ 0x%X, %d \},\n", hex($1), int($fatsz / hex($1)) );
                    $fatsz    -= hex($1) * int($fatsz / hex($1));
                    }
                    if ($fatsz > 0)
                    {
                    $template .= sprintf("   \{ 0x%X, %d \},\n", $fatsz , 1 );
            ...
            # Segment cross the FAT Baseaddr
            {
                ...
                if ($offset > 0 and (hex($1) - $offset) >= $fatsz)
                {
                    $template .= sprintf("   \{ 0x%X, %d \},\n", $fatsz, 1);
                    $fatsz    -= hex($1);
                    $c--;
...
#****************************************************************************
# subroutine:  additional_information_sync_access_settings
# input:       C104MHZ_SYNC_EMI / C52MHZ_SYNC_EMI
# return:      template of part "sync_access_settings" in "additional information"
#****************************************************************************
sub additional_information_sync_access_settings
{
    my ($NNNMHZ_SYNC_EMI) = @_;
    my $content;
    # extract the BB group information
    my $href = $part_info->{CS}->{0}->{$NNNMHZ_SYNC_EMI}->{BB_GROUP};
    my @BB = keys %$href;
    my $group_count;
    for ($group_count = 1, my $gname = sprintf("SETTING_G%d", $group_count);
         defined $part_info->{CS}->{0}->{$NNNMHZ_SYNC_EMI}->{$gname} ;
         $group_count++  , $gname = sprintf("SETTING_G%d", $group_count)  )
    {
    }
    $group_count--;
    # The Loop
    my $comment_string;
    for (my $group_idx = 1; $group_idx <= $group_count ; $group_idx++)
    {
        # The BB selection line
        $content .= ($group_idx == 1) ? "#if " : "#elif ";
        my $flag4OR = 0;
        foreach my $b (@BB)
        {
            next if ($part_info->{CS}->{0}->{$NNNMHZ_SYNC_EMI}->{BB_GROUP}->{$b} != $group_idx);
            $content .= sprintf("%sdefined(%s)", ($flag4OR) ? " || " : "", $b);
            $comment_string .= sprintf("%s%s", ($flag4OR) ? " || " : "", $b) if ($group_idx == 1);
            $flag4OR = 1;
        }
        $content .= "\n";
        #The setting body
        my $sgname = sprintf("SETTING_G%d", $group_idx);
        my $agname = sprintf("ADDRESS_G%d", $group_idx);

        my @addr = split /\s/, $part_info->{CS}->{0}->{$NNNMHZ_SYNC_EMI}->{$agname};
        my @data = split /\s/, $part_info->{CS}->{0}->{$NNNMHZ_SYNC_EMI}->{DATA};
        ...
        my @addr = split /\s/, $part_info->{CS}->{1}->{$NNNMHZ_SYNC_EMI}->{$agname};
        my @data = split /\s/, $part_info->{CS}->{1}->{$NNNMHZ_SYNC_EMI}->{DATA};
        ...
        my $emi_setting = $part_info->{CS}->{0}->{$NNNMHZ_SYNC_EMI}->{$sgname};
        $content .=  "#define EMI_FLASH_BURST_MODE        $emi_setting\n"; 

        $emi_setting = $part_info->{CS}->{1}->{$NNNMHZ_SYNC_EMI}->{$sgname};
        $content .=  "#define EMI_RAM_BURST_MODE        $emi_setting\n";
    }
    $content .= "#endif /* $comment_string */\n\n";
    # Done
    return $content;
}
...
sub additional_information
{
    ...
    {
        $content .= &additional_information_sync_access_settings('C104MHZ_SYNC_EMI');
...
#           $content .=  "#define EMI_DRV1_BURST_MODE        $driving_setting[1]\n";
    ...
    {
        $content .= &additional_information_sync_access_settings('C52MHZ_SYNC_EMI');
...
#                                                    MT6225 need only 1st and 3rd driving settings
        ...
        $content .=  "#elif ( defined(MT6223) || defined(MT6223P) )\n";
        my @driving_setting = split /\s/, $part_info->{CS}->{0}->{"52MHZ_SYNC_DRIVING"}->{MT6223};
        if ($driving_setting[0] ne 'x')
        {
            $content .=  "#define EMI_DRV0_BURST_MODE        $driving_setting[0]\n";
        }
        $content .=  "#elif ( defined(MT6227D) || defined(MT6226D) )\n";
        my @driving_setting = split /\s/, $part_info->{CS}->{0}->{"52MHZ_SYNC_DRIVING"}->{MT6227D};
        if ($driving_setting[0] ne 'x')
        {
            $content .=  "#define EMI_DRV0_BURST_MODE        $driving_setting[0]\n";
        }
...
sub DeviceListParser_NOR_RAM_MCP
{
    ...
    #                                                       "MT6227D"   => $setting,
    #                                                       "MT6226D"   => $setting, },
    #                           C52MHZ_SYNC_EMI     => {    BB_Group    => %hash_of_platforms,
    #                                                       SETTING_G1  => $setting,
    #                                                       SETTING_G2  => $setting,
    #                                                       ADDRESS_G1  => $address,
    #                                                       ADDRESS_G2  => $address,
    #                                                       DATA        => $data, },
    #                           C104MHZ_SYNC_EMI    => {    BB_Group    => %hash_of_platforms,
    #                                                       SETTING_G1  => $setting,
    #                                                       ADDRESS_G1  => $address,
    #                                                       DATA        => $data, },
    #                           C26MHZ_SYNC_EMI     => {    BB_Group    => %hash_of_platforms,
    #                                                       SETTING_G1  => $setting,
    #                                                       ADDRESS_G1  => $address,
    #                                                       DATA        => $data, },
    #                           C13MHZ_SYNC_EMI     => {    BB_Group    => %hash_of_platforms,
    #                                                       SETTING_G1  => $setting,
    #                                                       SETTING_G2  => $setting,
    #                                                       ADDRESS_G1  => $address,
    #                                                       ADDRESS_G2  => $address,
    ...
    foreach (@all_rows)
    {
        ...
        $part_info->{CS}->{$cs_value}->{FG}->{BANK_INFO}     = $Sheet->Cells($_, $COLUMN_BANK_INFO)->{'Value'};
        $part_info->{CS}->{$cs_value}->{FG}->{BLOCK_INFO}    = $Sheet->Cells($_, $COLUMN_BLOCK_INFO)->{'Value'};
        ...
        my $bbgroup_count = 0; # Build BB Group Table first
        while ((&xls_cell_value($Sheet, 1, $emi_setting_scan_idx) =~ /52MHz\s+Synchronous\s+EMI/i) &&
               ((my $platforms = &xls_cell_value($Sheet, 2, $emi_setting_scan_idx)) =~ /MT\d+/))
        {
            last if $platforms =~ /Address/i;
            while ($platforms =~ /(MT\w+)\W*/)
            {
                $part_info->{CS}->{$cs_value}->{C52MHZ_SYNC_EMI}->{BB_GROUP}->{$1}  = $bbgroup_count + 1;
                $platforms = $';
            }
            $emi_setting_scan_idx++;
            $bbgroup_count++;
        }
        $emi_setting_scan_idx      = $COLUMN_52MHZ_SYNC_EMI;
        for ( my $group_idx = 1 ;
                 $group_idx <= $bbgroup_count;
                 $group_idx++ )
        {
            my $setname     = sprintf("SETTING_G%d", $group_idx);
            my $emi_setting = &xls_cell_value($Sheet, $current_row, $emi_setting_scan_idx);
            $part_info->{CS}->{$cs_value}->{C52MHZ_SYNC_EMI}->{$setname}  = $emi_setting;
            $emi_setting_scan_idx++;
        }
        for ( my $group_idx = 1 ;
                 $group_idx <= $bbgroup_count;
                 $group_idx++ )
        {
            my $addrname    = sprintf("ADDRESS_G%d", $group_idx);
            my $address     = &xls_cell_value($Sheet, $current_row, $emi_setting_scan_idx);
            $part_info->{CS}->{$cs_value}->{C52MHZ_SYNC_EMI}->{$addrname}  = $address;
            $emi_setting_scan_idx++;
        }
        $part_info->{CS}->{$cs_value}->{C52MHZ_SYNC_EMI}->{DATA}       = &xls_cell_value($Sheet, $_, $emi_setting_scan_idx);
        die "NOR_RAM_MCP sheet header format incorrect 52MHZ_SYNC_EMI"
        unless (&xls_cell_value($Sheet, 1, $emi_setting_scan_idx) =~ /52MHz\s+Synchronous\s+EMI/i);
               ...
               ((my $platforms = &xls_cell_value($Sheet, 2, $emi_setting_scan_idx)) =~ /MT\d+/))
        {
            ...
            while ($platforms =~ /(MT\w+)\W*/)
            {
                $part_info->{CS}->{$cs_value}->{C104MHZ_SYNC_EMI}->{BB_GROUP}->{$1}  = $bbgroup_count + 1;
                $platforms = $';
            }
            $emi_setting_scan_idx++;
            $bbgroup_count++;
        }
        $emi_setting_scan_idx      = $COLUMN_104MHZ_SYNC_EMI;
        for ( my $group_idx = 1 ;
                 $group_idx <= $bbgroup_count;
                 $group_idx++ )
        {
            my $setname     = sprintf("SETTING_G%d", $group_idx);
            my $emi_setting = &xls_cell_value($Sheet, $current_row, $emi_setting_scan_idx);
            $part_info->{CS}->{$cs_value}->{C104MHZ_SYNC_EMI}->{$setname}  = $emi_setting;
            $emi_setting_scan_idx++;
        }
        for ( my $group_idx = 1 ;
                 $group_idx <= $bbgroup_count;
                 $group_idx++ )
        {
            my $addrname    = sprintf("ADDRESS_G%d", $group_idx);
            my $address     = &xls_cell_value($Sheet, $current_row, $emi_setting_scan_idx);
            $part_info->{CS}->{$cs_value}->{C104MHZ_SYNC_EMI}->{$addrname}  = $address;
            $emi_setting_scan_idx++;
        }
        $part_info->{CS}->{$cs_value}->{C104MHZ_SYNC_EMI}->{DATA}       = &xls_cell_value($Sheet, $_, $emi_setting_scan_idx);
        die "NOR_RAM_MCP sheet header format incorrect 104MHZ_SYNC_EMI"
        unless (&xls_cell_value($Sheet, 1, $emi_setting_scan_idx) =~ /104MHz\s+Synchronous\s+EMI/i);
               ...
               ((my $platforms = &xls_cell_value($Sheet, 2, $emi_setting_scan_idx)) =~ /MT\d+/))
        {
            ...
            while ($platforms =~ /(MT\w+)\W*/)
            {
                $part_info->{CS}->{$cs_value}->{C26MHZ_SYNC_EMI}->{BB_GROUP}->{$1}  = $bbgroup_count + 1;
                $platforms = $';
            }
            $emi_setting_scan_idx++;
            $bbgroup_count++;
        }
        $emi_setting_scan_idx      = $COLUMN_26MHZ_SYNC_EMI;
        for ( my $group_idx = 1 ;
                 $group_idx <= $bbgroup_count;
                 $group_idx++ )
        {
            my $setname     = sprintf("SETTING_G%d", $group_idx);
            my $emi_setting = &xls_cell_value($Sheet, $current_row, $emi_setting_scan_idx);
            $part_info->{CS}->{$cs_value}->{C26MHZ_SYNC_EMI}->{$setname}  = $emi_setting;
            $emi_setting_scan_idx++;
        }
        for ( my $group_idx = 1 ;
                 $group_idx <= $bbgroup_count;
                 $group_idx++ )
        {
            my $addrname    = sprintf("ADDRESS_G%d", $group_idx);
            my $address     = &xls_cell_value($Sheet, $current_row, $emi_setting_scan_idx);
            $part_info->{CS}->{$cs_value}->{C26MHZ_SYNC_EMI}->{$addrname}  = $address;
            $emi_setting_scan_idx++;
        }
        $part_info->{CS}->{$cs_value}->{C26MHZ_SYNC_EMI}->{DATA}       = &xls_cell_value($Sheet, $_, $emi_setting_scan_idx);
        die "NOR_RAM_MCP sheet header format incorrect 26MHZ_SYNC_EMI"
        unless (&xls_cell_value($Sheet, 1, $emi_setting_scan_idx) =~ /26MHz\s+Synchronous\s+EMI/i);
               ...
               ((my $platforms = &xls_cell_value($Sheet, 2, $emi_setting_scan_idx)) =~ /MT\d+/))
        {
            ...
            while ($platforms =~ /(MT\w+)\W*/)
            {
                $part_info->{CS}->{$cs_value}->{C13MHZ_SYNC_EMI}->{BB_GROUP}->{$1}  = $bbgroup_count + 1;
                $platforms = $';
            }
            $emi_setting_scan_idx++;
            $bbgroup_count++;
        }
        $emi_setting_scan_idx      = $COLUMN_13MHZ_SYNC_EMI;
        for ( my $group_idx = 1 ;
                 $group_idx <= $bbgroup_count;
                 $group_idx++ )
        {
            my $setname     = sprintf("SETTING_G%d", $group_idx);
            my $emi_setting = &xls_cell_value($Sheet, $current_row, $emi_setting_scan_idx);
            $part_info->{CS}->{$cs_value}->{C13MHZ_SYNC_EMI}->{$setname}  = $emi_setting;
            $emi_setting_scan_idx++;
        }
        for ( my $group_idx = 1 ;
                 $group_idx <= $bbgroup_count;
                 $group_idx++ )
        {
            my $addrname    = sprintf("ADDRESS_G%d", $group_idx);
            my $address     = &xls_cell_value($Sheet, $current_row, $emi_setting_scan_idx);
            $part_info->{CS}->{$cs_value}->{C13MHZ_SYNC_EMI}->{$addrname}  = $address;
            $emi_setting_scan_idx++;
        }
        $part_info->{CS}->{$cs_value}->{C13MHZ_SYNC_EMI}->{DATA}       = &xls_cell_value($Sheet, $_, $emi_setting_scan_idx);
        die "NOR_RAM_MCP sheet header format incorrect 13MHZ_SYNC_EMI"
        unless (&xls_cell_value($Sheet, 1, $emi_setting_scan_idx) =~ /13MHz\s+Synchronous\s+EMI/i);
...
#****************************************************************************
# subroutine:  Lookup_SYNC_EMI_setting_by_CS_CLK_BB
# input:       CHIP SECLECT , CLK , PLATFORM
# return:      None
#****************************************************************************
sub Lookup_SYNC_EMI_setting_by_CS_CLK_BB
{
    my ($cs, $clk, $bb) = @_;
    my $idx;
    $idx = $part_info->{CS}->{$cs}->{$clk}->{BB_GROUP}->{$bb};
    die "lookup $cs , $clk, $bb failure" unless defined $idx;

    my $sgname = sprintf("SETTING_G%d", $idx);
    return $part_info->{CS}->{$cs}->{$clk}->{$sgname};
}
...
# return:      None
#****************************************************************************
sub Lookup_SYNC_EMI_address_by_CS_CLK_BB
{
    my ($cs, $clk, $bb) = @_;
    my $idx;
    $idx = $part_info->{CS}->{$cs}->{$clk}->{BB_GROUP}->{$bb};
    ...
    my $sgname = sprintf("ADDRESS_G%d", $idx);
    return $part_info->{CS}->{$cs}->{$clk}->{$sgname};
}
...
sub FuncBody_custom_dynamicclockswitch_INTELSIBLEY
{
    my $C104M_Setting;
        ...
        my @emi_setting_6229 = split /\s/, $emi_setting_6229;
        $C104M_Setting .=  "      *(volatile kal_uint32 *)0x80010008 = $emi_setting_6229[0];\n";

        my @flash_addr = split /\s/, &Lookup_SYNC_EMI_address_by_CS_CLK_BB(0, 'C104MHZ_SYNC_EMI', "MT6229");
        my @flash_data = split /\s/, $part_info->{CS}->{0}->{C104MHZ_SYNC_EMI}->{DATA};
        ...
        my @ram_addr   = split /\s/, &Lookup_SYNC_EMI_address_by_CS_CLK_BB(1, 'C104MHZ_SYNC_EMI', "MT6229");
        my @ram_data   = split /\s/, $part_info->{CS}->{1}->{C104MHZ_SYNC_EMI}->{DATA};
        ...
        $emi_setting_6229 = &Lookup_SYNC_EMI_setting_by_CS_CLK_BB(0, 'C104MHZ_SYNC_EMI', "MT6229");
        $C104M_Setting .=  "      *(volatile kal_uint32 *)0x80010000 = $emi_setting_6229;\n";

        $C104M_Setting .=  "\n      /* delay for a while */\n      for (delay = 0; delay < 2; delay++) {\n\n         /* NOP */\n      }\n      /* set EMI B */\n";

        $emi_setting_6229 = &Lookup_SYNC_EMI_setting_by_CS_CLK_BB(1, 'C104MHZ_SYNC_EMI', "MT6229");
    ...
    my $C26M_Setting;
        ...
        $emi_setting_6229    = $part_info->{CS}->{1}->{ASYNC_EMI}->{MT6229};
        my @emi_setting_6229 = split /\s/, $emi_setting_6229;
        $C26M_Setting .=  "      *(volatile kal_uint32 *)0x80010008 = $emi_setting_6229[0];\n";

        my @flash_addr = split /\s/, &Lookup_SYNC_EMI_address_by_CS_CLK_BB(0, 'C26MHZ_SYNC_EMI', "MT6229");
        my @flash_data = split /\s/, $part_info->{CS}->{0}->{C26MHZ_SYNC_EMI}->{DATA};
        ...
        my @ram_addr   = split /\s/, &Lookup_SYNC_EMI_address_by_CS_CLK_BB(1, 'C26MHZ_SYNC_EMI', "MT6229");
        my @ram_data   = split /\s/, $part_info->{CS}->{1}->{C26MHZ_SYNC_EMI}->{DATA};
        ...
        $emi_setting_6229 = &Lookup_SYNC_EMI_setting_by_CS_CLK_BB(0, 'C26MHZ_SYNC_EMI', "MT6229");
        $C26M_Setting .=  "      *(volatile kal_uint32 *)0x80010000 = $emi_setting_6229;\n";

        $C26M_Setting .=  "\n      /* delay for a while */\n      for (delay = 0; delay < 2; delay++) {\n\n         /* NOP */\n      }\n      /* set EMI B */\n";

        $emi_setting_6229 = &Lookup_SYNC_EMI_setting_by_CS_CLK_BB(1, 'C26MHZ_SYNC_EMI', "MT6229");
    ...
    my $C13M_Setting;
        ...
        $emi_setting_6229    = $part_info->{CS}->{1}->{ASYNC_EMI}->{MT6229};
        my @emi_setting_6229 = split /\s/, $emi_setting_6229;
        $C13M_Setting .=  "      *(volatile kal_uint32 *)0x80010008 = $emi_setting_6229[0];\n";

        my @flash_addr = split /\s/, &Lookup_SYNC_EMI_address_by_CS_CLK_BB(0, 'C13MHZ_SYNC_EMI', "MT6229");
        my @flash_data = split /\s/, $part_info->{CS}->{0}->{C13MHZ_SYNC_EMI}->{DATA};
        ...
        my @ram_addr   = split /\s/, &Lookup_SYNC_EMI_address_by_CS_CLK_BB(1, 'C13MHZ_SYNC_EMI', "MT6229");
        my @ram_data   = split /\s/, $part_info->{CS}->{1}->{C13MHZ_SYNC_EMI}->{DATA};
        ...
        $emi_setting_6229 = &Lookup_SYNC_EMI_setting_by_CS_CLK_BB(0, 'C13MHZ_SYNC_EMI', "MT6229");
        $C13M_Setting .=  "      *(volatile kal_uint32 *)0x80010000 = $emi_setting_6229;\n";

        $C13M_Setting .=  "\n      /* delay for a while */\n      for (delay = 0; delay < 2; delay++) {\n\n         /* NOP */\n      }\n      /* set EMI B */\n";

        $emi_setting_6229 = &Lookup_SYNC_EMI_setting_by_CS_CLK_BB(1, 'C13MHZ_SYNC_EMI', "MT6229");
...
sub FuncBody_custom_dynamicclockswitch_LPSDRAM
{
    ...
    my $C104_Driving;
        ...
        $C104_Driving .=  "      *EMI_GENA = $ctrl_val | 0x80000000;\n";
    ###
    my $C26_Driving;
        ...
        $C26_Driving .=  "      *EMI_GENA = $ctrl_val | 0x80000000;\n";
    ...
    my $C13_Driving;
        ...
        $C13_Driving .=  "      *EMI_GENA = $ctrl_val | 0x80000000;\n";
    ###
    my $template = <<"__TEMPLATE";
...
sub FuncBody_custom_dynamicclockswitch_SYNC
{
   ...
   if (clock == MCU_104MHZ)
   {
   #error "SYNC_ACCESS on such platform Not support NON-SIBLEY series MCP yet"
   /* Manually configuration required if SYNC_ACCESS on such platform */
   }
#endif /* MT6228 || MT6225 || MT6229 || MT6230 || MT6268T */
...
#endif /* MT6223 || MT6223P || MT62227D || MT6226D */
...
#endif  /* MT6223 || MT6223P || MT6227D || MT6226D */
...
sub FuncBody_custom_dynamicclockswitch
{
           ...
           && ($FLASH_ACCESS_TYPE eq 'SYNC_ACCESS') && ($RAM_ACCESS_TYPE eq 'SYNC_ACCESS') )
    {
        ...
        {
            # assume MT6229 series only
            return &FuncBody_custom_dynamicclockswitch_INTELSIBLEY();
        } # if ($part_info->{CS}->{0}->{SERIES} eq 'INTEL_SIBLEY')
        else
        {
            return &FuncBody_custom_dynamicclockswitch_SYNC();
        }
    } # elsif ( ($MEMORY_DEVICE_TYPE eq 'NOR_RAM_MCP') && ... )
    elsif (  ($MEMORY_DEVICE_TYPE eq 'NOR_LPSDRAM_MCP')
          && ($FLASH_ACCESS_TYPE eq 'SYNC_ACCESS') )
...
sub FuncBody_custom_setAdvEMI_for_nor_ram_mcp
{
    ...
    && $BBtbl_SYNC_ACCESS{$PLATFORM} )
    {
        ...
        {
            $template .=  &custom_setadvemi_for_52MHz_sync_access();
...
sub HeaderBody_for_nor_ram_mcp
{
    ...
    ###
    my $bank_info_string = &bank_info( \@banks, $flash_size/8*1024*1024 - $sum_of_regions, $sum_of_regions );

    ###
    my $block_info_string   = $part_info->{CS}->{0}->{FG}->{BLOCK_INFO};
    ...
    if (defined $PROJECT_EXPECTED_RAMSIZE)
    {
       #ensure practical
       die "Cannot configure expected RAMSIZE larger than physical\n" 
       if ( hex($PROJECT_EXPECTED_RAMSIZE) > ($ram_size * 1024 * 1024 / 8) );

       #ensure 2^n
       my $i;
       for( $i = 1 ; $i < hex($PROJECT_EXPECTED_RAMSIZE) ; $i *= 2 ){1;}
       if ( $i < ($ram_size * 1024 * 1024 / 8) )
       {
          $optional_block_admux = "// PHYSICAL RAM DENSITY $ram_size Mb\n" . $optional_block_admux;
          $ram_size = $i * 8 / ( 1024 * 1024);
       }
    }
    if (defined $PROJECT_EXPECTED_CODESIZE)
    {
       my $offset;
       if (defined $PROJECT_EXPECTED_FATSIZE)
       {
          $offset = $flash_size/8*1024*1024 - hex($PROJECT_EXPECTED_FATSIZE);
       }
       else
       {
          $offset = $flash_size/8*1024*1024 - $sum_of_regions;
       }
       #ensure practical
       die "Cannot configure expected CODESIZE larger than physical\n" 
       if ( hex($PROJECT_EXPECTED_CODESIZE) > $offset );

       if ( hex($PROJECT_EXPECTED_CODESIZE) <= $offset )
       {
       	   if (hex($PROJECT_EXPECTED_CODESIZE) > ($flash_size/8*1024*1024 - $sum_of_regions))
       	   {
       	   	   $base_addr_string    = sprintf("0x%08X", hex($PROJECT_EXPECTED_CODESIZE));
       	   }
       #Don't modify FLASH_BASE_ADDRESS
       #add additional instruction for sysgen
           $base_addr_string .= "\n// PROJECT_EXPECTED_CODESIZE $PROJECT_EXPECTED_CODESIZE";
       }
    }
    if (defined $PROJECT_EXPECTED_FATSIZE)
    {
       #ensure practical
       die "Cannot auto configure expected FATSIZE larger than last bank\n" 
       if ( hex($PROJECT_EXPECTED_FATSIZE) > $sum_of_regions );

       if ( hex($PROJECT_EXPECTED_FATSIZE) < $sum_of_regions)
       {
          #ensure block alignment, reset blocks of regions
          my $new_fat_size  = 0;
          my $new_block_num = 0;
          my @new_regions;
          ...
          for (0..$#regions)
          {
              my $serial_block_num = 0;
              my $serial_block_sz;
              my $saved_block_count;
   
              if ($regions[$_] =~ /\{(0x\w+),\s*(\d+)\}/) # match {0x20000, 7}
              {
                  $serial_block_sz  = hex($1);
                  $serial_block_num = $2;
              }
   
              $saved_block_count = $serial_block_num;
              while ($serial_block_num > 0 and $new_fat_size < hex($PROJECT_EXPECTED_FATSIZE))
              {
                  $new_fat_size += $serial_block_sz;
                  $new_block_num++;
                  $serial_block_num--;
              }

              if ($saved_block_count > 0)
              {
                  $new_regions[$_] = sprintf "\{0x%x, %d\},", $serial_block_sz , ($saved_block_count - $serial_block_num);
              }
   
              last if ($new_fat_size >= hex($PROJECT_EXPECTED_FATSIZE));
          }
          #modify the flash settings
          $fat_space_string    = sprintf("0x%08X\n// ORIGINAL ALLOCATED 0x%08X", $new_fat_size, $sum_of_regions );
          $blocks_of_regions   = $new_block_num;
          $sub_content_block5  = &region_info(@new_regions);
          $esbnor_earse_queue  = 3;
          #the bank info need adjust, too
          if ((defined $PROJECT_EXPECTED_CODESIZE) && (hex($PROJECT_EXPECTED_CODESIZE)>($flash_size/8*1024*1024 - $sum_of_regions)))
          {
              $bank_info_string    = &bank_info(\@banks, hex($PROJECT_EXPECTED_CODESIZE) , $new_fat_size);
          }
          else
          {
              $bank_info_string    = &bank_info(\@banks, $flash_size/8*1024*1024 - $sum_of_regions , $new_fat_size);
          }
       }
    }
    ###
 ...
 *******************************************************************************/
#define SNOR_ERASE_QUEUE_SIZE   $esbnor_earse_queue 
