...
{
    mea_pckg_handle_t *pckg = 0;

    pckg = (mea_pckg_handle_t*) MSF_MEM_ALLOC(mod_id, sizeof(mea_pckg_handle_t));
    if (pckg == NULL)
    {
        return NULL;
    }
                       ...
                       void *sig_data)
{
    ...
    mea_pckg_inst_list_t *list = 0;
    mea_pckg_inst_list_t *inst = 0;
    mea_pckg_data_t      *private_data;
    MSF_INT32             oper_id = 0;
    msf_pck_result_t      oper_result;
    mea_pckg_result_t    *int_result = NULL;
    ...
    while (result == MSF_PACKAGE_SIGNAL_NOT_HANDLED && list != NULL)
    {
        ...
        if (result == MSF_PACKAGE_OPERATION_COMPLETE)
        {
            ...
            {                
                ...
                if (list->active_operations == 0)
                {
                    ...
                    mea_pckg_terminate(handle, list->pkg_inst, FALSE);
                }
            ...
            int_result = MSF_MEM_ALLOC(handle->mod_id, sizeof(mea_pckg_result_t));
            if (NULL == int_result)
            {
                list->pkg_data->package_fncs->free_result_fnc(list->pkg_inst, &oper_result);
            ...
            {
                mea_pckg_free_result(int_result);
            }
        ...
        else
        {
            ...
            {                
                mea_pckg_terminate(handle, inst->pkg_inst, FALSE);
            }
        }        
    }

    return result;
} /* mea_pckg_handle_signal() */
...
mea_pkg_id_t mea_pckg_reg_pckg(mea_pckg_handle_t* handle, const mea_pckg_fnc_list_t *fncs)
{
     ...
     */
    list = (mea_pckg_pkg_list_t*) MSF_MEM_ALLOC(handle->mod_id, sizeof(mea_pckg_pkg_list_t));
    if (list == NULL)
    {
        return 0;
    }
...
{
    mea_pckg_pkg_list_t *list = 0;
    mea_pckg_pkg_list_t *prev = NULL;
    mea_pckg_inst_list_t *inst = NULL;
...
mea_pkg_handle_t *mea_pckg_create_inst(mea_pckg_handle_t* handle, mea_pkg_id_t pckg_id)
{
     ...
     *	Allocate and link:
     */
    inst = (mea_pckg_inst_list_t*) MSF_MEM_ALLOC(handle->mod_id, sizeof(mea_pckg_inst_list_t));
    if (inst == NULL)
    {
        /*
...
{
    mea_pckg_inst_list_t  *inst = NULL;
    mea_pckg_inst_list_t  *prev = NULL;
     ...
     */
    if (MSF_PACKAGE_COMPLETED != inst->pkg_data->package_fncs->terminate_fnc(inst->pkg_inst))
    {
 ...
 */
int mea_pckg_cancel(mea_pckg_handle_t* handle, void* pkg_inst, MSF_INT32 id, void **private_data)
{
    mea_pckg_inst_list_t  *inst = NULL;
    mea_pckg_inst_list_t  *prev = NULL;
    mea_pckg_data_t       *int_data;
...
{
    msf_pck_result_t     oper_result;
    mea_pckg_data_t      *private_data = NULL;
    mea_pckg_inst_list_t *inst = NULL;
    mea_pckg_result_t    *result = NULL;
    int                   status;
    ...
    {
        result = MSF_MEM_ALLOC(handle->mod_id, sizeof(mea_pckg_result_t));
        if (NULL == result)
        {
    ...
    else /* operation delayed */
    {
         ...
         */
        private_data = MSF_MEM_ALLOC(handle->mod_id, sizeof(mea_pckg_data_t));
...
{
    mea_pckg_inst_list_t *inst_list = NULL;
    mea_pckg_pkg_list_t *pckg_list = NULL;
    
    inst_list = handle->reg_inst_list;
    while (NULL != inst_list)
    {
        mea_pckg_terminate(handle, inst_list->pkg_inst, TRUE);
        inst_list = handle->reg_inst_list;
    }
    ...
    {
        mea_pckg_unreg_pckg(handle, pckg_list->pckg_id);
        pckg_list = handle->reg_pkg_list;
    }
