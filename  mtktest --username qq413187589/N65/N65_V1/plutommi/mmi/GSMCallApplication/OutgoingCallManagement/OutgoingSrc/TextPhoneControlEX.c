...
void mmi_ctm_close_rsp_hdlr(void *info)
{
    ...
    if (ctm_p->state != MMI_CTM_STATE_CONNECTED)
    {
        ...
        {
            DeleteScreenIfPresent(SCR_CTM_TALKING);
...
void mmi_ctm_connect_rsp_hdlr(void *info)
{
    ...
    ctm_scr = OslMalloc(sizeof(history));
    ...
    if (ctm_p->state == MMI_CTM_STATE_CONNECTING)
    {
        if (result == TRUE)
        {
            ...
            else if(IsScreenPresent(SCR_ID_UCM_IN_CALL_OPTION))
            {
                ctm_scr->scrnID = SCR_CTM_TALKING;
                ctm_scr->entryFuncPtr = entry_ctm_talking;
                mmi_ucs2cpy((S8*) ctm_scr->inputBuffer, (S8*) & nHistory);
                InsertHistoryAfterThisScrnReference(SCR_ID_UCM_IN_CALL_OPTION, ctm_scr);
            }
            else if(IsScreenPresent(SCR_ID_UCM_IN_CALL))
            {
                ctm_scr->scrnID = SCR_CTM_TALKING;
                ctm_scr->entryFuncPtr = entry_ctm_talking;
                mmi_ucs2cpy((S8*) ctm_scr->inputBuffer, (S8*) & nHistory);
                InsertHistoryAfterThisScrnReference(SCR_ID_UCM_IN_CALL, ctm_scr);
            }
        }
        else
        {
            /* connect fail */
            mmi_ctm_set_state(MMI_CTM_STATE_OPENED);
            DisplayPopup((PU8) GetString(STR_GLOBAL_NOT_AVAILABLE), IMG_GLOBAL_ERROR, 0, UI_POPUP_NOTIFYDURATION_TIME, ERROR_TONE);
        }
    }
#ifdef __CTM_DEBUG
...
void mmi_ctm_connect_ind_hdlr(void *info)
{
    ...
    /* enter chat screen */
    if (GetExitScrnID() == SCR_ID_UCM_IN_CALL || GetExitScrnID() == SCR_ID_UCM_IN_CALL_OPTION)
    {
        entry_ctm_talking();
    }
    else if(IsScreenPresent(SCR_ID_UCM_IN_CALL_OPTION))
...
void mmi_ctm_connect(void)
{
    ...
    else if (ctm_p->state == MMI_CTM_STATE_CONNECTED)
    {
        ...
        if (IsScreenPresent(SCR_CTM_TALKING))
        {
            PRINT_INFORMATION("Already connected, go back to CTM talking screen");
...
void mmi_ctm_toggle_mode(void)
{
    ...
    DisplayPopup((PU8) GetString(str_id), IMG_GLOBAL_ACTIVATED, 1, UI_POPUP_NOTIFYDURATION_TIME, SUCCESS_TONE);
...
void entry_ctm_talking(void)
{
    ...
    SetRightSoftkeyFunction(mmi_ucm_end_single_action, KEY_EVENT_UP);
    SetKeyHandler(mmi_ucm_end_single_action, KEY_END, KEY_EVENT_DOWN);
}
