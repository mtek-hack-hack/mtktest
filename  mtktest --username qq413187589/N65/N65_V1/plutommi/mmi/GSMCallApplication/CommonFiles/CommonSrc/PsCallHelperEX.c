...
void MakePsSSVUSSDReq(U8 *MsgStruct, U8 *IPNum, U16 length, void *callBack)
{
    ...
    if (MsgStruct)
    {
        ...
        MTPNP_AD_Get_Channel(MTPNP_AD_CALL_CHANNEL) == MTPNP_AD_CHANNEL_DEFAULT_SLAVE)
        {
            ...
            strParse->source_id = MTPNP_AD_CC_GetSourceId();
            MTPNP_AD_CC_SetSourceId(LMMI_SRC);
        }
        else
    #endif /* __MMI_DUAL_SIM_MASTER__ */
        {
            Message.oslDestId = MOD_L4C;
            strParse->source_id = cm_p->ucm_source_id;
            cm_p->ucm_source_id = LMMI_SRC;
        }
    ...
    {
        SetSlaveProtocolEventHandler((PsFuncPtr)callBack, PRT_USSDVSS_RSP);
    }
#endif /* __MMI_DUAL_SIM_MASTER__ */
...
void MakePsCallAcceptIncoming(void *callBack)
{
    ...
    ataReq = (MMI_CC_ATA_REQ*) OslConstructDataPtr(sizeof(MMI_CC_ATA_REQ));
    
    ataReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
    
    Message.oslDataPtr = (oslParaType*) ataReq;
...
void MakePsAthRequest(void *callBack)
{
    ...
    athReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
...
void MakePsSendUDUB(void *callBack)
{
    ...
    udubReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
    ...
    /*auto reject doesn't need action rsp to UCM*/
...
void MakePsHangupallActiveAcceptOtherRequest(void *callBack)
{
    ...
    hangUp->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
...
/*    if (GetCurrentState() != CM_OUTGOING_STATE && GetCallState(handle) == CM_ACTIVE_STATE)
    ...
    hangupReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
...
void MakePsActiveHold(void *callBack)
{
    ...
    retrReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
...
void MakePsSplitRequest(CM_CALL_HANDLE nCallHandle)
{
    ...
    splitReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
...
void MakePsConfCallRequest(void)
{
    ...
    confReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
...
void MakePsEctRequest(void)
{
    ...
    ectReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
...
void MakePsSwapCallRequest(void *callBack)
{
    ...
    swapReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
...
void MakePsSendRelHeld(void *callBack)
{
    ...
    relHeldReq->source_id = cm_p->ucm_source_id;
    cm_p->ucm_source_id = LMMI_SRC;
...
void MakePsDropUSSDReq(PsFuncPtr callBack)
{
    ...
    if(cm_p->slave_ss_session == TRUE)
    {
        SetSlaveProtocolEventHandler(callBack, PRT_SS_STRING_ABORT_RSP);
    }
    else
#endif /* __MMI_DUAL_SIM_MASTER__ */
    {
...
void MakePsReverseSpeechStatusAux(pBOOL speechOn, U8 type)
{
    if (speechOn == KAL_TRUE)
    {
        ...
        {
            mmi_gsm_speech_ind(speechOn, MDI_AUDIO_SPEECH_APP_ID_GSM, MMI_UCM_VOICE_CALL_TYPE);
        }
        else if (type == MDI_AUDIO_SPEECH_APP_ID_WCDMA && mdi_audio_speech_get_app_id() != MDI_AUDIO_SPEECH_APP_ID_WCDMA)
        {
            mmi_gsm_speech_ind(speechOn, MDI_AUDIO_SPEECH_APP_ID_WCDMA, MMI_UCM_VOICE_CALL_TYPE);
        }
    }
    else
    {
        ...
        if ((type == MDI_AUDIO_SPEECH_APP_ID_WCDMA && 
...
void DeriveCallInfo(void *info, IncomingCall *incmg)
{
    ...
    /*pass to ucm for L4C information*/
    memcpy(&cm_p->l4c_number, &inc->num, sizeof(inc->num));
...
void DeriveCallInfoFromWait(void *info, IncomingCall *incmg)
{
    ...
    /*pass to ucm for L4C information*/
    memcpy(&cm_p->l4c_number, &wait->num, sizeof(wait->num));
...
*/
    ...
    else if(dcs == 0x11) /* If dcs is 0x11, skip the first two chars (lang indicator)*/
    {
        ...
        InputboxConvertGSMToDeviceEncoding(ussd_string+2, tempStr, ussrLen, (MAX_DIGITS_USSD * ENCODING_LENGTH), MMI_UCS2_DCS, 0, 1);
    }
    else
    {
        tempStr = OslMalloc(MAX_DIGITS_USSD * ENCODING_LENGTH);
...
void CheckFailureChld(void *info)
{
    ...
    if (CheckChldRequestSuccess(info, &result))
    {
        ...
        {
            if(chld_rsp->opcode == CSMCC_REL_SPECIFIC_CALL ||
                chld_rsp->opcode == CSMCC_REL_HELD_OR_UDUB)
            {
        ...
        else if(cm_p->ucm_operation == MMI_GSM_UCM_END_CONFERENCE&&
                cm_p->ucm_action_type == MMI_GSM_UCM_NORMAL)
        {
            mmi_gsm_end_conference_rsp(TRUE, NULL);
        }
        else if(cm_p->ucm_operation == MMI_GSM_UCM_AUTO_QUICK_END &&
                cm_p->ucm_action_type == MMI_GSM_UCM_CRSS)
        {
            mmi_gsm_stop_processing_ind(NULL, IMG_GLOBAL_OK, NULL);
        }
        return;
    }
    else
...
void CBackDropUSSDReqEND(void *info)
{
    ...
    {
        DisplayPopup((U8*) GetString(STR_GLOBAL_NOT_AVAILABLE), IMG_GLOBAL_ERROR, 0, 2000, (U8) ERROR_TONE);
    }
}
